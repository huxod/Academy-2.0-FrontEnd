...
export default class Login extends React.Component<any, any>{
    ...
    public validationsLogin = (ele: number) => {
    ...
    public validationsPassword = (ele: number) => {
        if (ele >= 9) { this.setState({ validPassword: 'Your password to short', valid: true }) }
        else { this.setState({ validPassword: 'Your password is correct', valid: false }) };
    }
    ...
    public onChangePassword = (e: any) => {
        ...
        this.validationsPassword(this.state.password.length);
    }
    public submit = async (e: any) => {
        ...
        await fetch(this.forms.action, {
            ...
            .catch(e => { console.log(e.responseText); this.setState({ validstate: 'login or password is not correct', signin: false }) });
        ...
        return (
            <Segment>
                <form className='ui form' method="POST" onSubmit={this.submit}
                    ...
                    <Form.Field>
                        <span>{this.state.validPassword}</span>
                ...
                {this.state.signin === true && this.state.valid === true && systemTool.ifLoged() ? 
                <Redirect to="/user" /> : this.state.validstate}
            </Segment>
        );
    }
}
